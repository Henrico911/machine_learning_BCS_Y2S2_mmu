...
def plot_decision_boundary(model, X, y, title="Decision Boundary"):
    """
    Plot decision boundary for a 2D SVM model
...
def main():
    ...
    accuracy_linear = accuracy_score(y_test_binary, y_pred_linear)
    ...
    # Train and visualize 2D model
    svm_2d = SVM(kernel=linear_kernel, C=1.0)
    svm_2d.fit(X_train_2d, y_train_binary_2d)
    y_pred_2d = svm_2d.predict(X_test_2d)
    accuracy_2d = accuracy_score(y_test_binary_2d, y_pred_2d)
    print(f"\nLinear Kernel 2D Accuracy: {accuracy_2d:.4f}")
    
    # Plot decision boundary
    plot_decision_boundary(svm_2d, X_train_2d, y_train_binary_2d, "SVM Decision Boundary (Linear Kernel)")
    ...
    # Evaluate the model
    y_pred_multi = ovr_svm.predict(X_test)
    accuracy_multi = accuracy_score(y_test_multi, y_pred_multi)
    print(f"One-vs-Rest SVM Accuracy: {accuracy_multi:.4f}")
